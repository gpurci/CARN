...
class ApplyWhiten2d(nn.Module):
   ...
   def reset_parameters(self):
      ...
      w *= 1. / w.std()
      #w = self.reduce_ch.weight.data
      #w *= 1. / w.std()

   def __transform(self, inputs, mean, std):
   ...
   def init_whiten(self, inputs, mean, std, eps=5e-4):
      inputs = self.__transform(inputs, mean, std)
   ...
   def forward(self, x, whiten_bias_grad=True):
      ...
      x = nn.functional.conv2d(x, self.whiten.weight, b if whiten_bias_grad else b.detach())
      #x = self.reduce_ch(x)
      x = self.model(x)
      return x / x.size(-1)
