...
class IdentityResNetUpBlock(nn.Module):
   ...
   def _unpack_blocks(self, key, **conf):
      ...
      for idx in range(num_residual_blocks):
         identity_upsample = None
         identity_downsample = None
         if (stride != 1):
            identity_upsample   = Conv2dUpSample(in_channels, kernel_size=3, stride=stride)
         if (in_channels != out_channels):
            identity_downsample = IdentityConv2dDownSample(in_channels, out_channels, stride=1)
         layer = IdentityUpResNet(in_channels, intermediate_channels, expansion=expansion, 
                                 identity_downsample=identity_downsample, identity_upsample=identity_upsample, 
                                 stride=1)
